...
gulp.task('app:offline', () => {
  ...
  if (env.is('local')) {
    util.log(`No need to set app offline in ${process.env.target} environment`)
    return
  }
...
gulp.task('app:online', () => {
  ...
  if (env.is('local')) {
    util.log(`No need to set app online in ${process.env.target} environment`)
    return
  }
...
gulp.task('app:recycle', () => {
  ...
  if (env.is('local')) {
    util.log(`No need to recycle app in ${process.env.target} environment`)
    return
  }
