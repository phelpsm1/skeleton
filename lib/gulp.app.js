...
gulp.task('app:offline', () => {
  ...
  util.log(util.colors.green(`Taking ${env.getTarget()} offline...`))
...
gulp.task('app:online', () => {
  ...
  util.log(util.colors.green(`Bringing ${env.getTarget()} online...`))
  ...
  env.getWebConfig().servers.forEach((server) => {
    ...
    let src = path.join(web.share, web.current.path, 'app_offline.htm')
