...
const notifyemail = () => {
  return getNotifyFileLatest()
    .then((latest) => {
      ...
      // need to verify this logic, if we are ever able to get list of commits again via gitlab api maybe?
      // if (lastDeployedRevision === '0000000') {
      //   commits = git.getCommits() // get all commits? What if it is just a new environment?
      // } else {
      //   commits = git.getCommits(lastDeployedRevision, process.env.revision)
      // }
...
const notifysquawk = (done) => {
  ...
  const body = {
    iapId: env.pkg.iapid,
    committer: process.env.USERNAME ? process.env.USERNAME.toLowerCase().replace('mfg_', '') : null,
    teamId: env.pkg.twcid,
    environment: process.env.target ? process.env.target.toLowerCase() : null,
    subApp: env.pkg.name,
    executor: 'manual',
    repoUrl: env.pkg.repository && env.pkg.repository.url ? env.pkg.repository.url.replace('.git', '') : null,
