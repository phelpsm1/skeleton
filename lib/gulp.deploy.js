...
gulp.task('deploy:notify:nr', () => {
  ...
  if (!envConfig.newrelic || !envConfig.newrelic.id) {
    log.warn(colors.yellow(`No New Relic Application Id defined to notify deployment to New Relic`))
    return
  }
  ...
  return rp(options)
    .then((response) => {
      log.info(colors.green(`Notified deployment to New Relic at ${response.deployment.timestamp}`))
    })
    .catch((err) => {
...
gulp.task('deploy:status', () => {
  ...
  return Promise.all(reads)
    ...
    .then((latest) => {
      latest.forEach((l) => {
        log.info(colors.green(`[${l.datetime}]\t[${String('     ' + l.environment).slice(-8)}]\tv${l.version}\t(${l.revision})\t${l.from}\t${l.by}`))
      })
    })
})
