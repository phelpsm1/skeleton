...
gulp.task('deploy:status', () => {
  let file = path.join(env.getWebConfig().servers.map((server) => env.getWebConfig(server).share)[0], 'deploy.log')
  ...
  return fs.readFile(file, 'utf8')
    ...
    .then((latest) => {
      latest.forEach((l) => {
        util.log(util.colors.green(`[${l.datetime}] [${String('     ' + l.environment).slice(-5)}] v${l.version} (${l.revision}) ${l.from} ${l.by}`))
      })
    })
})
