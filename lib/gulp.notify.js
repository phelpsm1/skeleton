  ...
  let mail = {
    ...
    html: `
      <html>
        ...
        <body>
          <p>
            <b>${env.pkg.name} v${env.pkg.version}</b> (${process.env.revision}) was deployed to <b>${process.env.target.toUpperCase()}</b> on <b>${moment().format('YYYY-MM-DD HH:mm:ss.SSS')}</b> by <b>${process.env.USERNAME}</b> from ${process.env.COMPUTERNAME}.
          </p>
        </body>
      </html>
    `
