...
function getWebConfig (server) {
  ...
  if (server) {
    ...
    web.current = {
      root: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, getName(), 'current'),
      dest: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, getName(), 'current', 'Web'),
      path: path.join('/', pkg.name, getName(), 'current', 'Web')
    }

    web.release = {
      root: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, getName(), 'releases'),
      dest: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, getName(), 'releases', process.env.revision, 'Web')
    }

    web.shared = {
      root: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, getName(), 'shared'),
      logs: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, getName(), 'shared', 'log')
    }
  }
