...
function parse () {
  ...
  // -r: get the revision off the command line, if provided, else fetch most recent commit from svn or git
  let revision
  let source
...
function getCcBuildLabel () {
  var label = process.env['CC_BUILD_LABEL']

  if(!label) {
    return;
  }

  var revision = label.substring(0, 7)
  var subBuild = label.match(/[.]\d+$/i) || []
