...
function parse () {
  ...
  if (!pkg.pillar) {
    throw new PluginError('env', 'application pillar is not defined in package.json')
  }
  ...
  const { revision, source } =
    ...
    (() => {
      if (args.r) {
        ...
        if (args.b.toString().match(isValidRevision) === null) {
          throw new PluginError('env', `revision ${args.b} is malformed`)
        }

        return { revision: args.b, source: 'local' }
      }
    ...
    (() => {
      try {
        const revision = git.revision.get()
