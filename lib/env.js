...
function getDbsConfig (name) {
  let target = name || process.env.database

  if (target) {
    return getEnvironmentConfig().dbs.find((db) => { return db.name === target })
  }

  return getEnvironmentConfig().dbs
}
...
function getWebConfig (server) {
  ...
  if (server) {
    ...
    web.release = {
      dest: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, getName(), 'release', process.env.revision, 'Web')
    }
  }
