...
function parse (argv, config) {
  ...
  if (args.e) {
    if (Object.keys(config.appSettings).includes(args.e)) {
      env = args.e
    } else {
      if (args.e === true) {
...
function getEnvironmentConfig (name) {
  ...
  return config.appSettings[name || process.env.target]
}
...
function getWebConfig (server) {
  ...
  if (server) {
    ...
    web.current = {
      root: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, process.env.target, 'current'),
      dest: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, process.env.target, 'current', 'Web'),
      path: path.join('/', pkg.name, process.env.target, 'current', 'Web')
    }

    web.release = {
      root: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, process.env.target, 'releases'),
      dest: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, process.env.target, 'releases', process.env.revision, 'Web')
    }

    web.shared = {
      root: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, process.env.target, 'shared'),
      logs: path.join(`//${server}`, `${pkg.pillar}$`, pkg.name, process.env.target, 'shared', 'log')
    }
  }
