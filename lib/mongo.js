...
function buildConnectionString (db) {
  if (db.user === undefined) {
    throw new Error(colors.red(`user value is missing from ${db}`))
  }

  if (db.password === undefined) {
    throw new Error(colors.red(`password value is missing from ${db}`))
  }

  if (db.servers === undefined || db.servers.length < 1) {
    throw new Error(colors.red(`servers value is missing/empty from ${db}`))
  }

  if (db.database === undefined) {
    throw new Error(colors.red(`database value is missing from ${db}`))
  }

  if (db.replicaSet === undefined) {
    throw new Error(colors.red(`replicaSet value is missing from ${db}`))
  }
