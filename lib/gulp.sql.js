...
const backup = () => {
  ...
  env.getDbsToBackup().forEach((db) => {
    const filename = path.join(db.backupShare, db.database, `${db.database}_backup_${moment().format('YYYY_MM_DD_HH_mm_ss')}.bak`)

    log.info(`Backing up ${db.name} (${db.database}) to ${filename} using SQL Server certificate ${process.env.sqlservercertificatename} ...`)
    ...
    sql += `ENCRYPTION(ALGORITHM = AES_256, SERVER CERTIFICATE = [${process.env.sqlservercertificatename}])`
    ...
    backups.push(
      mssql
        ...
        .catch(err => {
          log.error(colors.red(`ERROR: Failed to backup database: ${db.name} (${db.database})`), err)
          return err
        })
    )
  })
  ...
  return Promise.all(backups)
    .then(values => {
      if (values.some(v => v instanceof Error)) {
        throw values.find(v => v instanceof Error)
      }
    })
}
...
const restore = () => {
  ...
  env.getDbsToRestore().forEach((db) => {
    const restoreTo = db
    const restoreFrom = env.getDbsConfig(db.name, 'prod')

    // find the most recent SQL backup file
    const file = glob.sync('*.bak', { cwd: path.join(restoreFrom.backupShare, restoreFrom.database) }).sort().reverse()[0]
    ...
    const src = getFileInfo(path.join(restoreFrom.backupShare, restoreFrom.database), file)
    const local = getFileInfo(path.join('c:', 'temp', restoreFrom.database), file)

    const filename = env.is('local') ? local.path : src.path
    ...
    const restore = () => mssql
      ...
      .catch(err => {
        log.error(colors.red(`ERROR: Failed to restore database: ${db.name} (${db.database})`), err)
        return err
      })
    ...
    if (env.is('local') && !fs.existsSync(local.path)) {
      restores.push(
        fs.copy(src.path, filename)
          ...
          .catch(err => {
            log.error(colors.red(`ERROR: Failed to copy ${src.path} to ${filename}`), err)
          })
      )
    } else {
