...
const info = (done) => {
  log.info(colors.gray('process.env:'))

  for (const key in process.env) {
    log.info(colors.gray(`\t${key}: ${process.env[key]}`))
  }

  done()
}

info.displayName = 'process:info'
info.description = 'Display process environment variables'
gulp.task(info)
