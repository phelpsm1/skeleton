...
function setAppSettings (json, configuration, pkg) {
  ...
  const environment = process.env.target
...
function setLog4NetAppenderSetting (appenders, name, key, value) {
  const appender = appenders.find(i => i.$.name === name)
...
function setLog4NetAppenderSettings (json, configuration) {
  ...
  const appenders = json.configuration.log4net[0].appender
  ...
  config.log4net.appenders.forEach((appender) => {
    ...
    Object.keys(appender).filter((key) => key !== 'name').forEach((key) => {
      if (key.toLowerCase() === 'connectionstring') {
        const db = configuration.dbs.find((db) => { return db.name === appender[key] })
...
function setLog4NetLoggerSetting (loggers, name, key, value) {
  const logger = (name !== 0) ? loggers.find(i => i.$.name === name) : loggers[0]
...
function setLog4NetLoggerSettings (json, configuration) {
  ...
  const root = json.configuration.log4net[0].root
  ...
  const loggers = json.configuration.log4net[0].logger
...
function setThrottleSettings (json, settings) {
  const config = settings.config
  const key = 'throttlePolicy'

  if (!config) {
    log.warn(colors.yellow(`No ${key} set due to no appSettings["<env>"]["config"] configuration data defined in package.json`))
    return
  }
  ...
  Object.keys(config[key]).forEach((k) => {
    if (json.configuration[key] === undefined) {
      log.warn(colors.yellow(`Could not find ${key} in configuration file`))
      return
    }

    if (json.configuration[key][0] !== Object(json.configuration[key][0])) {
      json.configuration[key][0] = { $: {} }
    }

    log.info(`......${k} to ${config[key][k]}`)
    json.configuration[key][0].$[k] = config[key][k]
  })
}
...
function updateWebConfig (file, dest, configuration, pkg) {
  ...
  const json = xmlToJson(file)

  setAppSettings(json, configuration, pkg)
  setThrottleSettings(json, configuration)
...
function getNotifyFileLatest (filename, parser) {
  ...
  // find all files for each defined environment
  definedEnvironments.forEach((e) => {
    const config = env.getEnvironmentConfig(e)
  ...
  // create promises that read each file for all defined environments
  const reads = []
...
module.exports = {
  cfg: {
    ...
    setThrottleSettings: setThrottleSettings,
