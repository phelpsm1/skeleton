...
function setConnectionStrings (json, configuration, pkg) {
  ...
  if (!database) {
    util.log(util.colors.red(`No ConnectionStrings set due to no appSettings["<env>"]["db"]["database"] defined in package.json`))
    return
  }

  util.log(`...connectionStrings...`)

  setConnectionString(adds, name, buildConnectionString(server, database))
}
...
function updateWebConfig (file, dest, configuration, pkg) {
  ...
  util.log(`Updating ${file.relative} to ${env.getName()} settings...`)
