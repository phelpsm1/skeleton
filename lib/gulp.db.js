...
gulp.task('db:backup', () => {
  ...
  env.getDbsConfig().forEach((db) => {
    ...
    let filename = path.join(db.backupPath, db.database, `${db.database}_backup_${moment().format('YYYY_MM_DD_HH_mm_ss')}.bak`)
...
gulp.task('db:restore', () => {
  ...
  let filename = path.join(restoreFrom.backupPath, restoreFrom.database, glob.sync('*.bak', {cwd: path.join(restoreFrom.backupPath, restoreFrom.database)}).sort().reverse()[0])

  util.log(`Restoring database ${restoreTo.database} in ${env.getName()} from file ${filename}...`)
