...
const notifylog = (done) => {
  if (env.is('local')) {
    log.warn(colors.yellow(`No need to perform task in ${process.env.target} environment.`))
    done()
    return
  }

  const pattern = '[%d] [%5x{target}] %h %x{user} '
  const tokens = {
...
function getNotifyFileLatest () {
  const parser = (line) => {
    const matches = /\[(.*)\]\s*\[\s*(\S*)\]\s*(\S*)\s*(\S*)/igm.exec(line)
